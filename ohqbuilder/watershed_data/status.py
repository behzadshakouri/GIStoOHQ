...
def select_catalog_asset(
    *, catalog: str | Path, product: str, processing_status: str = "native",
) -> str:
    """Return the newest catalog asset ID matching an explicit product and status."""
    if not product.strip():
        raise WatershedDataError("asset selection requires a product name")
    if processing_status not in {"native", "derived"}:
        raise WatershedDataError("asset selection status must be native or derived")
    matches = [
        asset for asset in AssetCatalog(catalog).read()["assets"]
        if asset.get("product") == product
        and asset.get("processing_status") == processing_status
    ]
    if not matches:
        raise WatershedDataError(
            f"catalog has no {processing_status} asset for product {product!r}"
        )
