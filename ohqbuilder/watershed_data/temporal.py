...
TEMPORAL_QC_POLICY_VERSION = "temporal-qc-v5"
...
EXPECTED_UNITS = {
    "00060": {"ft3/s", "ft^3/s", "m3/s", "m^3/s"},
    "PRECTOTCORR": {"mm/day", "mm/hour", "mm/hr"},
    "T2M": {"C", "degC", "°C"}, "RH2M": {"%", "percent"}, "WS2M": {"m/s"},
    "ALLSKY_SFC_SW_DWN": {
        "MJ/hr", "MJ/m^2/day", "MJ/m^2/hour", "kW-hr/m^2", "kWh/m^2",
    },
...
def harmonize_asset(
    *, asset_id: str, catalog: str | Path, object_store: str | Path,
    ...
    if fail_on_qc_error and failed_error_rules:
        diagnostics = []
        for result in qc:
            if result.rule_id == "temporal.unit_compatibility" and not result.passed:
                diagnostics.extend(
                    f"{item['variable']}={item['actual_unit']} "
                    f"(allowed: {', '.join(item['allowed_units'])})"
                    for item in result.details["mismatches"]
                )
    ...
    output = catalog_store.register({
        ...
        "product_version": "1.6", "request_key": hashlib.sha256(
        ...
        "transformation_name": "native-to-utc-table", "transformation_version": "1.7",
