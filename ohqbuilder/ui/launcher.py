...
from ohqbuilder.watershed_data.status import select_catalog_asset
...
class LauncherApp:
    ...
    def configure_watershed_data(self) -> None:
        ...
        variables = {
            ...
            "Asset product": tk.StringVar(value="historical-meteorology"),
        ...
        def run(action: str) -> None:
            ...
            if action == "use-catalog-asset":
                try:
                    asset_id = select_catalog_asset(
                        catalog=variables["Catalog"].get(),
                        product=variables["Asset product"].get(),
                    )
                except WatershedDataError as exc:
                    self.messages.put(f"ERROR: {exc}\n")
                    return
                variables["Native asset ID"].set(asset_id)
                self.messages.put(f"Selected native catalog asset: {asset_id}\n")
                return
            try:
