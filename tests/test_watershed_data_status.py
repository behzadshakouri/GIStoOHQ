...
from ohqbuilder.watershed_data.status import (
    build_data_status, select_catalog_asset, write_data_status,
)
...
def test_status_lists_asset_ids_counts_and_object_availability(tmp_path):
    ...
    assert select_catalog_asset(catalog=catalog.path, product="weather") == native["asset_id"]
