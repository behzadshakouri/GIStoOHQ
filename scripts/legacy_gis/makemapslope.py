# =============================================================================
# make_map_slope.py   (QGIS Python Console)
#
# NHA WS3 -- automated report figure: SLOPE (percent) for one site, with the
# watershed boundary drawn as an outline (no fill). Reads the clipped slope
# raster (outputs/clipped/slope_pct.tif) produced by the pipeline. Assembles a
# print layout with subtitle, legend, north arrow, and scale bar; exports a
# 300-dpi PNG cropped to content.
#
# Sibling of make_map_dem.py -- same layout machinery, different raster + ramp.
#
# USAGE (QGIS Python Console):
#   exec(open("/home/arash/Dropbox/Chloeta/NHA/PythonScripts/make_map_slope.py").read())
#
...
# Slope raster (percent): clipped slope from the pipeline.
RASTER_PATH   = os.path.join(OUT_DIR, "clipped", "slope_pct.tif")
# Watershed boundary polygon (outline only).
BOUNDARY_PATH = os.path.join(OUT_DIR, "watershed_boundary.gpkg")
# Output PNG.
OUT_PNG = os.path.join(SITE_DIR, "figures", "%s_slope.png" % SITE.replace("-", "_"))

TITLE_TEXT  = "%s -- Digital Elevation Model" % SITE
SUBTITLE    = "NHA WS3 Flood Mitigation -- Slope (percent), derived from USGS 3DEP"
DPI         = 300
...
# slope often has a long tail (a few cliff cells) that washes out the ramp.
# stretch from 0 to mean + 2*stdev (capped at the true max) so the bulk of the
# basin uses the full color range. Set STRETCH_TO_MAX = True to use raw max.
STRETCH_TO_MAX = False
vmin = 0.0
if STRETCH_TO_MAX:
    vmax = float(int(round(vmax)))
else:
    hi = stats.mean + 2.0 * stats.stdDev
    vmax = float(int(round(min(hi, stats.maximumValue))))
    if vmax <= vmin:
        vmax = float(int(round(stats.maximumValue)))

# slope ramp: flat = pale yellow, rising through orange to dark red/brown
stops = [
    (0.00, QColor(255, 255, 204)),  # flat   - pale yellow
    (0.25, QColor(254, 217, 142)),
    (0.50, QColor(254, 153, 41)),   # moderate - orange
    (0.75, QColor(217, 95, 14)),
    (1.00, QColor(140, 45, 4)),     # steep  - dark red/brown
]
...
rl.triggerRepaint()
# legend label: show "Slope (%)" instead of "Band 1 (Gray)"
rl.setName("Slope (%)")
...
# hide the auto "Band 1 (Gray)" sublabel so only "Slope (%)" shows
try:
